...
- **Terminators keep their positions; the lines swap between them.** Do *not* move each line together with its own terminator — that model has to invent a terminator whenever the piece landing first is the unterminated last line, and inventing one means writing `doc.eol` over a break the user never touched. On a `.Mixed` document that silently converts a CRLF to an LF somewhere the user did not edit.
  Under the position-preserving model the separator between the two pieces is *always the one that already separated them*, which always exists, and the terminator after the region is empty exactly when the region ends the buffer unterminated. **No synthesis, ever** — so the operation is a pure relocation of bytes and cannot alter any line ending. A `.Mixed` fixture is the test that distinguishes the two models; on a uniform LF or CRLF document they are byte-identical, which is why nothing else catches it.
- **Bound the operation.** `pt_line_start` and `pt_line_end` are uncapped, and this path is not navigation — it is scan **plus** allocate **plus** copy **plus** insert. On a multi-GB single-line file, two uncapped backward scans run *before* the `lo == 0` bail even fires, so the no-op is the expensive case. Use the capped helpers with an edit budget well above `RENDER_LINE_CAP` (1–4 MB), and treat a cap hit as a **bail, not a truncation** — refusing to move a pathologically long line is correct; splitting one would corrupt it.
